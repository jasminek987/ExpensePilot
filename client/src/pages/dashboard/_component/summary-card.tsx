...
const getCardStatus = (value: number, cardType: CardType, expenseRatio?: number): CardStatus => {
  if (cardType === "savings") {
    if (value === 0) return { label: "No Savings Record", color: "text-gray-400", Icon: TrendingDownIcon };
    if (value < 10) return { label: "Low Savings", color: "text-red-500", Icon: TrendingDownIcon, description: `Only ${value.toFixed(1)}% saved` };
    if (value < 20) return { label: "Moderate", color: "text-yellow-500", Icon: TrendingDownIcon, description: `${expenseRatio?.toFixed(0)}% spent` };
    if (expenseRatio && expenseRatio > 75) return { label: "High Spend", color: "text-red-500", Icon: TrendingDownIcon, description: `${expenseRatio.toFixed(0)}% spent` };
    if (expenseRatio && expenseRatio > 60) return { label: "Warning: High Spend", color: "text-orange-500", Icon: TrendingDownIcon, description: `${expenseRatio.toFixed(0)}% spent` };
    return { label: "Good Savings", color: "text-green-600", Icon: TrendingUpIcon };
  }

  if (value === 0) {
    const typeLabel = cardType === "income" ? "Income" : cardType === "expenses" ? "Expenses" : "Balance";
    return { label: `No ${typeLabel}`, color: "text-gray-400", Icon: TrendingDownIcon, description: "" };
  }

  if (cardType === "balance" && value < 0) {
    return { label: "Overdrawn", color: "text-red-600", Icon: TrendingDownIcon, description: "Balance is negative" };
  }
...
const cardAccentColor: Record<CardType, string> = {
  balance: "border-t-blue-500",
  income: "border-t-green-500",
  expenses: "border-t-red-500",
  savings: "border-t-yellow-500",
};
...
const SummaryCard: FC<SummaryCardProps> = ({
  ...
  if (isLoading) {
    return (
      <Card className={cn("!gap-0 border-t-2", cardAccentColor[cardType])}>
  ...
  return (
    <Card className={cn("!gap-0 border-t-2", cardAccentColor[cardType])}>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 !pb-3">
        <CardTitle className="text-[13px] uppercase tracking-widest text-muted-foreground font-medium">
          {title}
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className={cn(
          "text-3xl font-bold",
          cardType === "balance" && value < 0 ? "text-red-600" : "text-foreground"
        )}>
          <CountUp
            start={0}
            end={value}
        ...
        <div className="text-sm text-muted-foreground">
          {cardType === "savings" ? (
            <div className="flex items-center gap-1.5">
              ...
              {status.description && <span className="text-muted-foreground ml-1">• {status.description}</span>}
            </div>
            ...
            <div className="flex items-center gap-1.5">
              ...
              {!status.description && <span className="text-muted-foreground">• {dateRange?.label}</span>}
            </div>
          ) : showTrend ? (
            <div className="flex items-center gap-1.5">
              {percentageChange !== 0 && (
                <div className={cn("flex items-center gap-0.5", trendDirection === "positive" ? "text-green-600" : "text-red-600")}>
